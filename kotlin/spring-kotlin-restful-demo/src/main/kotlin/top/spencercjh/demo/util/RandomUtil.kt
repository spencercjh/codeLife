...
object RandomUtil {
    ...
    private val emailSuffix = "@gmail.com,@yahoo.com,@msn.com,@hotmail.com,@aol.com,@ask.com,@live.com,@qq.com,@0355.net,@163.com,@163.net,@263.net,@3721.net,@yeah.net,@googlemail.com,@126.com,@sina.com,@sohu.com,@yahoo.com.cn".split(",")
    ...
    private fun getSex(): Student.Sex {
        return if (threadLocalRandom.nextBoolean()) Student.Sex.Male else Student.Sex.Female
    }

    /**
     * If [sex] isn't passed in, there is a random sex from [getSex]
     * @param sex Sex
     * @return a random Chinese name
     */
    fun getName(sex: Student.Sex = getSex()): String {
        val stringBuffer = StringBuffer()
     ...
     */
    fun getStudents(amount: Int, clazz: Clazz): List<Student> {
        if (amount <= 0) {
        ...
        for (i in 1..amount) {
            val sex = getSex()
            students.add(Student(name = getName(sex), phone = getPhone(), email = getEmail(), sex = sex, clazz = clazz))
        }
        return students
    }
}