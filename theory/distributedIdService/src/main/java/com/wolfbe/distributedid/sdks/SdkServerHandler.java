...
public class SdkServerHandler extends SimpleChannelInboundHandler {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof SdkProto) {
            ...
            if (semaphore.tryAcquire(GlobalConfig.ACQUIRE_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    semaphore.release();
                    logger.error("Sdk Server handler error", e);
                }
            } else {
                sdkProto.setDid(-1);
                ...
                String info = String.format("SdkServerHandler tryAcquire semaphore timeout, %dms, waiting thread " +
                        ...
                        GlobalConfig.ACQUIRE_TIMEOUT_MILLIS,
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        ...
        logger.error("Sdk Server Handler channel [{}] error and will be closed", NettyUtil.parseRemoteAddr(channel), cause);
